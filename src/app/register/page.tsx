...
function PasswordStrengthBar({ password }: { password: string }) {
	...
	return (
		<div className="space-y-1.5" aria-label={meta.label}>
			...
			<p
				className={`text-xs ${score <= 2 ? "text-red-400" : score === 3 ? "text-yellow-400" : "text-green-400"}`}
			>
				{meta.label}
			</p>
		</div>
...
function RegisterForm() {
	...
	const schema = z
		.object({
			name: z
				.string()
				.min(1, r.errors.nameRequired)
				.min(2, r.errors.nameTooShort),
			email: z
				.string()
				.min(1, r.errors.emailRequired)
				.pipe(z.email(r.errors.emailInvalid)),
			password: z
				.string()
	...
	const onSubmit = async (data: FormValues) => {
		...
		try {
			...
			const needsVerification =
				!result.data?.token && !result.data?.user?.emailVerified;
						...
						>
							...
							<div className="space-y-1.5">
								...
								{confirmPassword && !errors.confirmPassword && (
									<p
										className={`text-xs ${confirmPassword === password ? "text-green-600" : "text-red-500"}`}
									>
