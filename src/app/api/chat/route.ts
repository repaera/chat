...
function extractImageTags(text: string): {
	cleanedText: string;
	imageUrls: string[];
} {
...
export async function POST(req: Request) {
	...
	if (lastMessage?.role === "user") {
		...
		const customPartsData = (lastMessage.parts ?? []).filter(
			(p: any) => p.type === "location" || p.type === "commute",
		);
		...
		await db.message
			.create({
				data: {
					...
					content: userText,
					customParts:
						customPartsData.length > 0 ? JSON.stringify(customPartsData) : null,
				},
			})
			.catch((err: unknown) => {
		...
		if (!convTitle && userText.length > 0) {
			await db.conversation.update({
				where: { id: convId },
				data: {
					title: userText.slice(0, 80) + (userText.length > 80 ? "…" : ""),
				},
			});
		}
	}
	...
	if (process.env.MCP_URL && process.env.MCP_APPS_URL) {
		return Response.json(
			{
				error:
					"Misconfiguration: set either MCP_URL or MCP_APPS_URL, not both.",
			},
			{ status: 500 },
		);
	}
	...
	const MAX_TOOL_RESULT_CHARS = parseInt(
		process.env.MAX_TOOL_RESULT_CHARS ?? "3000",
		10,
	);
	const messagesForLLM = messages.slice(-contextWindow).map((msg, i, arr) => {
		const parts = ((msg.parts ?? []) as any[]).map((p: any) => {
			...
			if (
				...
				return {
					...p,
					result: `${p.result.slice(0, MAX_TOOL_RESULT_CHARS)}\n[truncated]`,
				};
			}
			return p;
		});
		...
		if (i === arr.length - 1 && msg.role === "user" && currentTime) {
			return {
				...msg,
				parts: [
					{ type: "text" as const, text: `[datetime: ${currentTime}]` },
					...newParts,
				],
			};
		}
	...
	const conversationHasImages =
		...
		messages.some((m) =>
			(m.parts ?? []).some(
				(p: any) =>
					p.type === "file" && (p.mediaType as string)?.startsWith("image/"),
			),
		);
			...
			const summary = {
				...
				cause:
					error instanceof Error && error.cause
						...
						: undefined,
				responseBody:
					(error as any)?.responseBody ?? (error as any)?.body ?? undefined,
			};
		...
		onFinish: async ({ messages: finishedMessages }) => {
				...
				mcpClients.map((client) =>
					client.close().catch((err) =>
						Sentry.captureException(err, {
							tags: { source: "mcp:close:onFinish" },
						}),
					),
				),
			);
			...
			try {
				...
				await db.conversation.update({
					...
					data: {
						updatedAt: new Date(),
						...(imageParts.length > 0 || imageUrls.length > 0
							? { hasImages: true }
							: {}),
					},
				});
