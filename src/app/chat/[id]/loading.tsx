...
export default function ChatLoading() {
  return (
    <div className="flex h-svh overflow-hidden bg-neutral-950">
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden bg-white">
        {/* Topbar */}
        <header className="flex h-12 shrink-0 items-center gap-2 px-3 border-b border-neutral-100">
          <div className="w-6 h-6 rounded bg-neutral-200 animate-pulse" />
          <div className="w-32 h-3.5 rounded bg-neutral-200 animate-pulse" />
        </header>
...
function SkeletonBubble({
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser && (
        <div className="shrink-0 w-7 h-7 rounded-lg bg-neutral-200 animate-pulse mt-0.5" />
      )}
      <div
        className={`rounded-2xl px-3.5 py-2.5 flex flex-col gap-2 ${
          isUser ? "bg-neutral-100" : "bg-neutral-900"
        }`}
        ...
        {Array.from({ length: lines }).map((_, i) => (
          <div
            ...
            className={`h-3 rounded animate-pulse ${
              isUser ? "bg-neutral-300" : "bg-neutral-700"
            }`}
