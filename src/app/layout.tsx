...
import { GtmNoScript, GtmScript } from "@/components/providers/gtm";
...
export async function generateMetadata(): Promise<Metadata> {
	...
	return {
		...
		icons: {
			...
			icon: [
				...(appConfig.iconSvg
					? [{ url: appConfig.iconSvg, type: "image/svg+xml" }]
					: [{ url: "/icon.svg", type: "image/svg+xml" }]),
				...(appConfig.icon192
					? [{ url: appConfig.icon192, type: "image/png", sizes: "192x192" }]
...
export default async function RootLayout({
	...
	return (
		<html
			lang={resolved.ui.meta.htmlLang}
			translate="no"
				...
				{mapsKey && appConfig.locationMode === "v2" && (
					<script
						...
						src={[
							...
							appConfig.googleMapsRegion
								? `&region=${appConfig.googleMapsRegion}`
								: "",
