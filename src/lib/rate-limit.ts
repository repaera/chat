...
function checkMemory(id: string, opts: { limit: number; windowMs: number }): RateLimitResult {
	const now = Date.now();
...
async function checkRedis(id: string, opts: { limit: number; windowMs: number }): Promise<RateLimitResult> {
	const now = Date.now();
