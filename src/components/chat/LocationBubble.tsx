...
import { MapPin, ArrowRightLeft, Navigation } from "lucide-react";
...
const LeafletMapInner = dynamic(() => import("@/components/chat/LeafletMapInner"), {
  ssr: false,
  loading: () => (
...
export function LocationBubble({ part }: { part: LocationPart }) {
  ...
  return (
    <div className="mt-1 flex flex-col gap-1.5">
      <a href={gmapsUrl} target="_blank" rel="noopener noreferrer" className="block w-full hover:opacity-90 transition-opacity">
        <LeafletMapInner
          center={[part.lat, part.lng]}
          zoom={15}
          markers={[{ lat: part.lat, lng: part.lng, label: part.label, color: "blue" }]}
        />
      </a>
    ...
    <div className="mt-1 flex flex-col gap-1.5">
      <a href={gmapsUrl} target="_blank" rel="noopener noreferrer" className="block w-full hover:opacity-90 transition-opacity">
        <LeafletMapInner
          ...
          zoom={12}
          markers={[
            { lat: part.origin.lat,      lng: part.origin.lng,      label: part.origin.label,      color: "green" },
            { lat: part.destination.lat, lng: part.destination.lng, label: part.destination.label, color: "red"   },
          ]}
          fitBounds
        />
      </a>
      <div className="flex flex-col gap-0.5">
        <div className="flex items-start gap-1.5">
          ...
          <p className="text-xs text-neutral-500 leading-snug truncate">{part.origin.label}</p>
        </div>
        <div className="flex items-start gap-1.5">
          <div className="w-2 h-2 rounded-full bg-red-500 shrink-0 mt-1" />
          <p className="text-xs text-neutral-500 leading-snug truncate">{part.destination.label}</p>
        </div>
      </div>
