...
import { ArrowLeft, Check, Copy, Unlink } from "lucide-react";
...
export default function SettingsClient({
	...
	const [saving, setSaving] = useState<
		null | "profile" | "email" | "locale" | "password"
	>(null);
	const [emailSent, setEmailSent] = useState(false);
	const [locale, setLocale] = useState(user.locale ?? "");

	// Links tab state
	const [linksStatus, setLinksStatus] = useState<Record<
		string,
		LinkStatus
	> | null>(null);
	...
	useEffect(() => {
		if (section === "links" && !linksStatus) {
			fetch("/api/link/status")
				.then((r) => r.json())
				.then((data: { links: Record<string, LinkStatus> }) =>
					setLinksStatus(data.links),
				)
				.catch(() => {});
		}
	}, [section, linksStatus]);
	...
	const handleUnlink = async (platform: string) => {
		...
		try {
			...
			setLinksStatus((prev) =>
				prev ? { ...prev, [platform]: { linked: false, handle: "" } } : prev,
			);
	...
	const handleSaveLocale = async (value: string) => {
		...
		setSaving("locale");
		try {
			await authClient.updateUser({ locale: value } as Parameters<
	...
	const handleChangePassword = async () => {
		if (newPassword.length < 8) {
			toast.error(s.securityCard.errors.tooShort);
			return;
		}
		setSaving("password");
		try {
	...
	const navItems: { key: Section; label: string; danger?: boolean }[] = [
		...
		...(availablePlatforms.length > 0
			? [{ key: "links" as const, label: s.tabLinks }]
			: []),
	];
					...
					<div className="space-y-8">
						...
						<div className="space-y-3 max-w-sm">
							...
							<Button
								...
								disabled={saving === "profile" || name === user.name}
							>
								{saving === "profile"
									? t.common.saving
									: s.profileCard.saveNameButton}
							</Button>
						</div>
							...
							<>
								...
								<div className="space-y-3 max-w-sm">
									...
									<Button
										...
										disabled={
											saving === "email" || email === user.email || emailSent
										}
										variant="outline"
									>
										{saving === "email"
											? s.profileCard.changingEmail
											: s.profileCard.changeEmailButton}
									</Button>
								</div>
							</>
						)}
								...
								>
									...
									<SelectContent>
										<SelectItem value="__auto__">
											{s.profileCard.localeAuto}
										</SelectItem>
										{(
											Object.entries(s.profileCard.localeOptions) as [
												string,
												string,
											][]
										).map(([key, label]) => (
					...
					<div className="space-y-8">
						...
						<div>
							<h2 className="text-base font-semibold">
								{s.securityCard.title}
							</h2>
							<p className="text-sm text-muted-foreground mt-0.5">
								{s.securityCard.description}
							</p>
						</div>
						...
						<div className="space-y-4 max-w-sm">
							...
							<Button
								...
								disabled={
									saving === "password" || !currentPassword || !newPassword
								}
							>
								{saving === "password"
									? s.securityCard.submittingButton
									: s.securityCard.submitButton}
							</Button>
						</div>
					</div>
				)}
					...
					<div className="space-y-8">
						...
						<div className="space-y-4">
							{availablePlatforms.map((platform) => {
								...
								const platformLabel =
									(s.linksCard.platforms as Record<string, string>)[platform] ??
									platform;
											...
											{status?.linked ? (
												<AlertDialog>
													...
													<AlertDialogContent>
														<AlertDialogHeader>
															<AlertDialogTitle>
																{s.linksCard.unlinkConfirmTitle}
															</AlertDialogTitle>
															<AlertDialogDescription>
																{s.linksCard.unlinkConfirmDescription}
															</AlertDialogDescription>
														</AlertDialogHeader>
														<AlertDialogFooter>
															<AlertDialogCancel>
																{s.linksCard.unlinkConfirmCancel}
															</AlertDialogCancel>
															<AlertDialogAction
																onClick={() => handleUnlink(platform)}
															>
																{s.linksCard.unlinkConfirmDelete}
															</AlertDialogAction>
														</AlertDialogFooter>
													</AlertDialogContent>
												</AlertDialog>
											...
											<div className="bg-muted rounded-md p-3 space-y-2">
												...
												<div className="flex items-center gap-2">
													...
													<Button
														...
														size="sm"
														onClick={() =>
															handleCopyCode(platform, codeEntry.code)
														}
													>
														{copied === platform ? (
														...
														<span className="ml-1 text-xs">
															{s.linksCard.copyCode}
														</span>
													</Button>
												</div>
												<p className="text-xs text-muted-foreground">
													{s.linksCard.codeExpiry}
												</p>
												<p className="text-xs text-muted-foreground/70">
													{s.linksCard.refreshHint}
												</p>
											</div>
										)}
									</div>
				...
				{section === "account" && (
					<div className="space-y-8">
						...
						<div>
							...
							<p className="text-sm text-muted-foreground mt-0.5">
								{s.retentionNotice.replace("{days}", String(retentionDays))}
							</p>
						</div>
						...
						<div className="space-y-3 max-w-sm">
							<div>
								...
								<p className="text-xs text-muted-foreground mt-1">
									{s.dangerCard.description}
								</p>
							</div>
							<AlertDialog>
								<AlertDialogTrigger asChild>
									<Button variant="destructive">
										{s.dangerCard.deleteButton}
									</Button>
								</AlertDialogTrigger>
								<AlertDialogContent>
									<AlertDialogHeader>
										<AlertDialogTitle>
											{s.dangerCard.confirmTitle}
										</AlertDialogTitle>
										<AlertDialogDescription>
											{s.dangerCard.confirmDescription}
										</AlertDialogDescription>
									</AlertDialogHeader>
									<AlertDialogFooter>
										<AlertDialogCancel>
											{s.dangerCard.confirmCancel}
										</AlertDialogCancel>
										<AlertDialogAction
											onClick={handleDeleteAccount}
											className="bg-red-600 hover:bg-red-700 text-white"
										>
											{s.dangerCard.confirmDelete}
										</AlertDialogAction>
									</AlertDialogFooter>
								</AlertDialogContent>
							</AlertDialog>
