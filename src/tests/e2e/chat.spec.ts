...
async function loginAs(page: Page, email = USER.email, password = USER.password) {
...
test.describe.serial("Chat flow", () => {
  ...
  test("new conversation appears in the sidebar", async ({ page }) => {
    await loginAs(page);
    await page.getByPlaceholder(/type a message/i).fill("Tell me something interesting");
    await page.locator("button[type='submit']").click();
    await expect(page).toHaveURL(/\/chat\/.+/);

    await openSidebar(page);
    await expect(page.locator("aside")).toContainText(/tell me|conversation/i);
  });
  ...
  test("delete a conversation returns to /", async ({ page }) => {
    ...
    await page.getByPlaceholder(/type a message/i).fill("Test delete conversation");
    await page.locator("button[type='submit']").click();
    await expect(page).toHaveURL(/\/chat\/.+/);
