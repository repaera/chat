...
describe("chatRequestSchema", () => {
	it("accepts valid message with text part", () => {
		const result = chatRequestSchema.safeParse({
			messages: [
				{ id: "1", role: "user", parts: [{ type: "text", text: "halo" }] },
			],
		});
...
describe("createConversationSchema", () => {
	...
	it("accepts valid title", () => {
		expect(
			createConversationSchema.safeParse({ title: "Todo today" }).success,
		).toBe(true);
	});

	it("rejects empty title", () => {
		expect(createConversationSchema.safeParse({ title: "" }).success).toBe(
			false,
		);
	});
