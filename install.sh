...
generate_secrets() {
	if [[ -z "${BETTER_AUTH_SECRET:-}" ]]; then
		...
		success "Generated BETTER_AUTH_SECRET and POSTGRES_PASSWORD"
	else
		success "Using provided BETTER_AUTH_SECRET; generated POSTGRES_PASSWORD"
	fi
...
main() {
	...
	# Finalise step count now that CLOUDFLARE_PROXY is known:
	#   Always:      secrets, clone, write-files, build, db, migrate, start, verify = 8
	#   Direct SSL:  +ssl, configure-https, cert-renewal = 3
	if [[ "$CLOUDFLARE_PROXY" == "true" ]]; then
		TOTAL_STEPS=$((STEP + 8))
	else
		TOTAL_STEPS=$((STEP + 11))
